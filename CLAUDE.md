...
- **One Language**: The repository is English — code, comments, docs, logs, commit messages, service responses. Three exceptions, all content: `apps/dashboard/src/app/lib/i18n/catalog-de.ts`; `apps/dashboard/public/offline.html`, which prints both languages because it is served without the catalogue; and the `label_de`/`labelDe` metric labels in the registry and its generated catalogue. The imprint and privacy policy are not among them: the repository ships no legal text at all, the operator writes both in the dashboard, and a template must not be committed back. A German comment is a defect.
...
- **No Value Is Silently Dropped**: A provider field is stored, or carried in `metadata`, or named in the field report — never ignored. An unexpected shape gets interpreted (an array of interval sums *is* the session's sum), and a derived number says so via `derived_from` / `derived_by` / `sample_count`. What the provider stated beats what we derived. Never write per-sample values into a `SUM` metric whose daily total already arrives — that double counts, and a wrong number is worse than a missing one. Every point carries `metadata.provider_value` and `metadata.units`; whole raw payloads are not stored.
