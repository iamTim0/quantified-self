...
function Loading({ message }: { message: MessageKey }) {
  const t = useT();
  return (
...
const TrendChart = dynamic(() => import("./TrendChart"), {
  ssr: false,
  loading: () => <Loading message="overview.loadingChart" />,
});

const LocationMap = dynamic(() => import("./LocationMap"), {
  ssr: false,
  loading: () => <Loading message="overview.loadingMap" />,
});
...
export default function OverviewTab({
  ...
  const todayFormatted = new Date().toLocaleDateString(intlLocale, {
    weekday: "long",
    day: "2-digit",
    month: "long",
    year: "numeric",
      ...
      <div>
        {hasData ? (
          ...
          <div className="glass-card p-10 text-center bg-white border border-slate-200 rounded-3xl">
            <p className="text-sm font-medium text-slate-500 mb-4">{t("overview.empty")}</p>
            <button
