...
14. **No Personal or Environment-Specific Information in the Repository**: This repository is intended to be published. Nothing committed to it may identify the person who runs it, the machine it was written on, or where it is deployed. Specifically forbidden anywhere in tracked files — source, tests, fixtures, documentation, compose files, commit messages, planning notes:
    - **Real email addresses.** Use `@example.com` or `@example.test`. Copyright notices are the exception, because a notice with the name taken out is not a notice: the author line in `NOTICE`, and the third-party licence texts this project is obliged to redistribute (`apps/dashboard/THIRD-PARTY-NOTICES.txt`, `apps/dashboard/licenses/`). Both are listed explicitly in `.agents/scripts/check_private_info.py` — an allowance is a decision that gets written down, not a pattern that quietly stops matching. `LICENSE` is deliberately not among them: it holds the FSF's wording and no name, because GitHub identifies a licence by matching that file against the known text and a copyright header in front of it made this repository report "no license". See [Licensing](docs/licensing.md).
...
16. **One Language in the Repository**: Everything committed here is written in **English** — code, identifiers, comments, docstrings, documentation, log lines, commit messages, test names, and every string a service returns. There are exactly three deliberate exceptions, all of them content rather than code, and all listed here because an allowance is a decision that gets written down (rule 14):
    - `apps/dashboard/src/app/lib/i18n/catalog-de.ts`, the German half of the interface catalogue.
    - `apps/dashboard/public/offline.html`, which prints both languages at once. It is served by the service worker when the network is gone, so it can reach neither the catalogue nor a stylesheet — and the catalogue is precisely the thing that would have known the reader's language. Choosing one language here is not possible; showing both is. Each half carries its own `lang` attribute so a screen reader pronounces it correctly. Do not "fix" this file by deleting its German half.
    - The `label_de` fields in the metric registry (`packages/shared-schemas/src/shared_schemas/metrics.py`) and the `labelDe` fields in the catalogue generated from it (`apps/dashboard/src/app/lib/metrics/catalog.ts`). A metric's display name is content in exactly the way a catalogue entry is, and it cannot live in `catalog-de.ts`: the registry is the single source for metric names (rule 15) and the dashboard's copy is **generated** by `task metrics:generate`, so hand-editing the German out of either would be undone by the next generation and fail the staleness check in `task test:packages`. Add or change a German label in the registry and regenerate — never in the generated file.

    **The legal documents are no longer among these exceptions, because this repository no longer contains one.** The imprint and the privacy policy are written by the operator in the dashboard and stored in `legal_documents` (see [Legal texts](docs/features/legal-documents.md)); that text is a deployment's own content, written at runtime, and this rule does not reach it. The German templates that used to live in `apps/dashboard/src/app/legal/` are deleted, and nothing may reintroduce them: a shipped notice full of `[placeholder]` markers publishes a company that does not operate the service, which is a wrong notice rather than a missing one, and a reader cannot tell the two apart. A deployment that has written nothing gets a page saying exactly that. What does carry over is the part that is about meaning rather than about the repository: **the German version governs**, so Core refuses to store English text for a document whose German text is empty, and an English reader with no English version is shown the German one under a note.
...
- ❌ A non-English string outside the exceptions rule 16 names (the German catalogue, `offline.html`, the registry's `label_de`) — a German comment counts, and a legal template committed back into `apps/dashboard/src/app/legal/` counts twice.
