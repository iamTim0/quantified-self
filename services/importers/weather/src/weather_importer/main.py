...
async def credentials(tenant_id: str, request_id: str) -> dict[str, Any] | None:
    ...
    async with httpx.AsyncClient(timeout=10) as client:
        response=await client.get(f"{settings.CORE_SERVICE_URL}/api/v1/internal/data/sources/weather/token",headers=headers)
...
async def main() -> None:
    ...
    await stream.subscribe("qs.task.sync.weather",queue="weather_importer_task_group",cb=lambda msg: process(msg,connection))
