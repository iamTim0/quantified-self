...
async def process(message: Any, connection: Any) -> None:
    try:
        ...
        logger.info(f"[req_id={request_id}] Processing calendar sync task for tenant {tenant_id}")
        ...
        if secret and secret.get("access_token") and secret.get("status") == "active":
            ...
            if base_url:
                ...
                for event in transform(records, tenant_id, secret["source_id"]):
                    ...
                    published += 1
                logger.info(f"[req_id={request_id}] Published {published} calendar events to JetStream")
    except Exception as e:
