"""Request-driven home_assistant importer publishing exclusively through JetStream."""
import asyncio, json
...
async def credentials(tenant_id: str, request_id: str) -> dict[str, Any] | None:
    ...
    async with httpx.AsyncClient(timeout=10) as client:
        response=await client.get(f"{settings.CORE_SERVICE_URL}/api/v1/internal/data/sources/home_assistant/token",headers=headers)
...
async def main() -> None:
    ...
    await stream.subscribe("qs.task.sync.home_assistant",queue="home_assistant_importer_task_group",cb=lambda msg: process(msg,connection))
