...
# ── Which address the sign-in throttle counts against ────────────────────────


def _request_from(headers: dict[str, str], peer: str | None = "10.0.0.5"):
    """A Starlette request with a chosen socket peer and header set."""
...
def test_the_client_address_is_taken_from_the_right_of_the_chain(monkeypatch):
    """The leftmost X-Forwarded-For entry is whatever the caller typed.

    Every hop appends and nobody verifies, so a throttle keyed on the left is
    bypassed by sending a header — which is worse than no throttle, because it
    reads as coverage. With one proxy in front, the last entry is the peer
    Traefik actually accepted.
    """
    from gateway.config import settings
    from gateway.main import _client_address

    monkeypatch.setattr(settings, "TRUSTED_PROXY_HOPS", 1)
    request = _request_from({"X-Forwarded-For": "1.1.1.1, 203.0.113.9"})
    assert _client_address(request) == "203.0.113.9"


def test_a_spoofed_chain_cannot_choose_its_own_bucket(monkeypatch):
    """The attacker controls everything left of the real entry, and none of it counts."""
    from gateway.config import settings
    from gateway.main import _client_address

    monkeypatch.setattr(settings, "TRUSTED_PROXY_HOPS", 1)
    forged = _request_from(
        {"X-Forwarded-For": "9.9.9.9, 8.8.8.8, 7.7.7.7, 198.51.100.4"}
    )
    # Whatever the caller prepends, the rightmost entry is still what the proxy saw.
    assert _client_address(forged) == "198.51.100.4"


def test_a_second_proxy_shifts_which_entry_is_trusted(monkeypatch):
    """Behind two proxies the last entry is the inner proxy, not the caller."""
    from gateway.config import settings
    from gateway.main import _client_address

    monkeypatch.setattr(settings, "TRUSTED_PROXY_HOPS", 2)
    request = _request_from({"X-Forwarded-For": "203.0.113.9, 10.0.0.2"})
    assert _client_address(request) == "203.0.113.9"


def test_with_no_proxy_configured_only_the_socket_peer_counts(monkeypatch):
    """A header cannot override the connection when nothing is in front."""
    from gateway.config import settings
    from gateway.main import _client_address

    monkeypatch.setattr(settings, "TRUSTED_PROXY_HOPS", 0)
    request = _request_from({"X-Forwarded-For": "1.2.3.4"}, peer="10.0.0.5")
    assert _client_address(request) == "10.0.0.5"


def test_a_short_chain_falls_back_to_the_peer(monkeypatch):
    """Fewer entries than configured hops means the header is not what we expect.

    Trusting it anyway would read the caller's own value. The socket peer is
    wrong in a harmless direction — one shared bucket for a misconfiguration —
    where the alternative is wrong in the direction that grants a bypass.
    """
    from gateway.config import settings
    from gateway.main import _client_address
...
def test_the_client_ip_header_is_never_relayed_from_the_caller():
    """It is assigned by the Gateway, so it must not be in the forward allowlist.

    If it were, a caller could name their own bucket and the per-client ceiling
    would count nothing.
    """
    from gateway.main import _SAFE_FORWARD_HEADERS, CLIENT_IP_HEADER

    assert CLIENT_IP_HEADER.lower() not in _SAFE_FORWARD_HEADERS
